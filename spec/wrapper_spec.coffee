...
describe 'given 123 56 89', ->
  ...
  it "wrapped to 9 returns unchanged string", ->
    expect(wrap(line, 9).changed).toBeFalsy()

  it "wrapped to 8 returns 123 56#{wi}89", ->
    expect(wrap(line, 8).line).toEqual("123 56#{wi}89")

  it "wrapped to 8 returns changed string", ->
    expect(wrap(line, 8).changed).toBeTruthy()
